(function(win, options, undefined){
	...
	var hideKefu = true										//是否隐藏人工客服
		...
		, apiUrl = 'http://127.0.0.1:8002/api/getResponse/'		//机器人接口地址
		, cssUrl = '/static/robot/css/robot.css'						//远程要加载的css文件地址
		, kfJsUrl = '/static/robot/ddp-client.js'						//人工客服库文件
		, kfServerHost = '#'				//人工客服服务器
		, kfTimeout = 12									//客服请求超时时间，毫秒
	...
	Robot.prototype = {
		...
		, 'send': function(text){
			...
			_this.scroll();
			_jsonp({//apiUrl + _con.appid +'/answer'
				'url': apiUrl
...
//					// add
//					var bm_html;
//					bm_html = '<ul class="smartnlp-msg-choices clearfix">';
//					for(var i=0,sugs = data.suggestion,len=sugs.length; i<len; i++){
//							bm_html += '<li class="smartnlp-check-item" rel="'+ sugs[i] +'">&nbsp;'+ sugs[i] +'</li>';
//						};
//					bm_html += '</ul>';
//					$('.'+ cssfix +'msg-bm').html(bm_html).on('click', '.'+ cssfix +'sug-item', function(){
...
//					// add
	...
	//新窗口打开新页面的URL,手机版有效
	, 'newurl': '?mobile'
	, 'logo': '/static/robot/logo.png'
	, headImg: '/static/robot/logo.png'
	, 'feedBtn': {
		//反馈的链接地址
		'url': '#'
		//反馈颜色背景
